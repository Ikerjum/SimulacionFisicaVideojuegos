...
public:
    ...
    Defense();
    Defense(Vector3 vel, Vector3 pos, Vector3 acc, float mass, Vector4 color, PxReal tam, float timeOfLife);
    // Constructor de copia de datos simple (opcional, pero útil)
    Defense(const Defense& other);
    Defense& operator=(const Defense& other);
    ...
    Defense* clone(PxReal tam = 0) const;
